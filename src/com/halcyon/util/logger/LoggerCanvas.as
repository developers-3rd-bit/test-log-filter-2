package com.halcyon.util.logger
{
   ...
   public class LoggerCanvas extends HalcyonCanvas
   {
      ...
      private var _filterText:TextInput;
      ...
      public function LoggerCanvas(reference:DisplayObjectContainer, argWidth:Number=15, argHeight:Number=15)
      {
         ...
         // filter
         _filterText = new TextInput();
         _filterText.width = 180;
         _filterText.addEventListener(Event.CHANGE, onFilterTextChange, false, 0, true);
         this.prepareElementAndPosition(_filterText, 12, NaN, 160, NaN);
         ...
         _list.addEventListener(LoggerList.ITEM_SEARCHING_COMPLETED, onItemSearchComplete, false, 0, true);
         this.prepareElementAndPosition(_list, 60, NaN, LIST_TO_BORDER, NaN);
      }
      
      private function onItemSearchComplete(layoutEvent:LayoutEvent):void {
         var hasData:Boolean = layoutEvent.extra as Boolean;
         if(hasData)
         {
            if(this.contains(_list)) return;
            this.addChild(_list);
         }
         else 
         {
            if(this.contains(_list) == false) return;
            this.removeChild(_list);
         }
      }
      
      public function onFilterTextChange(event:Event):void 
      {
         _list.searchItem(_filterText.text);
      }
